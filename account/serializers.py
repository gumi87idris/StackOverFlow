...
class RegisterSerializer(serializers.ModelSerializer):
    password = serializers.CharField(min_length=8, write_only=True, required=True)  # required=True - обязателен для заполнения
    ...
    def validate(self, attrs):      # attrs (не очищенные данные) -> validated_data (очищенные данные)
        password = attrs.get('password')    # attrs - словарь веденые данные пользователем все поля
